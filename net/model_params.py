...
class DGIR(nn.Module):
    def __init__(self,
        ...
        self.encoder_level1 = get_clones(
            TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                             LayerNorm_type=LayerNorm_type), self.num_blocks[0])
        # self.encoder_cross1 = get_clones(DCAM(dim), self.num_blocks[0])

        # encoder layer 2
        self.down1_2 = Downsample(dim)  ## From Level 1 to Level 2
        self.encoder_level2 = get_clones(
        ...
        self.encoder_level3 = get_clones(
        ...
        self.latent = get_clones(
            TransformerBlock(dim=int(dim * 2 ** 3), num_heads=heads[3], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type), self.num_blocks[3])
        ...
        self.decoder_level3 = get_clones(
            TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type), self.num_blocks[2])
        ...
        self.decoder_level2 = get_clones(
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type), self.num_blocks[1])
        ...
        self.decoder_level1 = get_clones(
        ...
        # last layer
        self.refinement_dc = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
    ...
    def forward(self, inp_img):
        ...
        oe_dc1 = self.tb2(self.encoder_level1, ie_dc1)
        ...
        oe_dc2 = self.tb2(self.encoder_level2, ie_dc2)
        ...
        oe_dc3 = self.tb2(self.encoder_level3, ie_dc3)
        ...
        latent_d2c = self.tb2(self.latent, ie_dc4)
        ...
        od_dc3 = self.tb2(self.decoder_level3, id_dc3)
        ...
        od_dc2 = self.tb2(self.decoder_level2, id_dc2)
        ...
        od_dc1 = self.tb2(self.decoder_level1, id_dc1)
...
if __name__ == "__main__":
    ...
    model = DGIR().cuda()
    tmp_0 = model(torch.rand(1, 3, 256, 256).cuda())  ### torch.rand(1, 3, 224, 224
    print(tmp_0.shape)

    summary(model, (1, 3, 256, 256))  # summary的函数内部参数形式与导入的第三方库有关，否则报错
